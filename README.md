...
- `NetHostSync.psm1` —— hosts 文本变换模块（可选，缺失时自动回退内置逻辑；有/无/模块出错三种情况都能正常更新 hosts）
...
- 请务必将**部署目录**（含 4 个脚本/模块与 `config.json`）加入对应安全软件的**信任列表 / 白名单**。
- 否则排查时会误以为"功能没生效"，其实只是被杀软挡了。
...
> **手动设置优先**：菜单 `2`（静态）/ `3`（DHCP）/ `6`（恢复备份）会记录该网卡的模式到本机 `manual_override.json`。此后自动切换遇到有 override 的网卡**一律跳过、完全不动它**，不会再被"有线强制静态"覆盖回去。想交回自动管理，执行一次菜单 `1`（会清空 override）。
